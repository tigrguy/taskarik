...
var appData = {
    ...
    'currentBoard': 0,  // id открытой доски
    'identifier': 0
};
...
function renderBoard(board) {
    ...
    document.title = 'taskarik | ' + currentBoard().name;
...
function addBoard() {
    ...
    if (!_boardTitle) return createAlert("поле пустое");  
    if (appData.boards.length >= 512) return createAlert("слишком много досок.")  
...
class Card {
    ...
    renderItems() {
        ...
        for (let _item of this.items) {
            ...
            _newItemDeleteButton.addEventListener('click', () => {
                createConfirmDialog("хочешь удалить задачу?", () => this.removeItem(_item));
            });
    ...
    generateElement() {
        ...
        _newButton.addEventListener('click', () => {
            ...
            if (!_inputValue) return createAlert("поле пустое");
...
class Board {
    ...
    addCard() {
        ...
        if (!_cardTitle) _cardTitle = `без имени  ${this.cards.length + 1}`;
...
const cardContextMenu_clearCard = () => {
    createConfirmDialog('хочешь очистить карточку?', () => {
...
const cardContextMenu_deleteCard = () => {
    createConfirmDialog('хочешь удалить карточку?', () => {
        let _currentCardObject = getCardFromElement(cardContextMenu_currentCard);
...
function loadData() {
    ...
    if (_data) {
        ...
        let _defaultBoard = new Board("доска", 'b0', {'theme': null});
...
e_saveButton.addEventListener('click', () => {saveData(); createAlert("успешно сохранено")});

e_deleteButton.addEventListener('click', () => {
    createConfirmDialog('хочешь удалить доску??', () => {
        ...
        createAlert(`доска удалена "${_boardName}"`)
    });
});
